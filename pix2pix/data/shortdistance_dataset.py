...
class ShortDistanceDataset(BaseDataset):
    ...
    def __init__(self, opt, transform=None):
        ...
        self.depthData_transform = transforms.Compose([
    ...
    def __getitem__(self, index):
        ...
        depth_map_path = os.path.join(self.dir_depth_map, self.rgb_paths[index].replace("jpg", "png"))
        data_rgb = read_image_data(rgb_path)  # needs to be a tensor
        data_depth_map = read_image_data(depth_map_path)  # needs to be a tensor
        # Do depth map transform
        data_depth_map = self.depthData_transform(data_depth_map)
